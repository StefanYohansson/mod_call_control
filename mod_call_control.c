...
static void webhook_event_handler(switch_event_t *event)
{
	...
	CURLcode res;
	long rescode;
	ks_json_t *json_res = ks_json_create_object();
	ks_json_t *json_event = ks_json_create_object();
	char *json_str = NULL;
	char *req = NULL;
	...
	if ((task = switch_core_hash_find(globals.tasks_hash, uuid))) {
		...
		if (should_dispatch) {
			...
			if (switch_event_serialize_json_obj(event, &json_event) == SWITCH_STATUS_SUCCESS) {
				ks_json_add_string_to_object(json_res, "uuid", uuid);
				ks_json_add_item_to_object(json_res, "event", json_event);

				json_str = ks_json_print_unformatted(json_res);
				ks_json_delete(&json_event);
				ks_json_delete(&json_res);
				...
				if ((res = switch_curl_easy_perform(curl))) {
					...
					if (rescode != 200) {
						switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Event delivery failed with HTTP code %ld, %s\n", rescode, rd.data);
